...
class _RegulatorProfileScreenState extends State<RegulatorProfileScreen> {
  final RegulatorNavTab _currentTab = RegulatorNavTab.profile;
  bool _isLoading = true;
  bool _isSigningOut = false;

  String _officerName = 'Regulatory Officer';
  String _officerEmail = 'officer@gov.in';
  String _departmentName = 'Department of Consumer Affairs, Legal Metrology';
    ...
    try {
      ...
      if (user != null) {
        if (user.email != null && user.email!.isNotEmpty) {
          _officerEmail = user.email!;
        }
        final metadata = user.userMetadata;
        if (metadata != null && metadata['full_name'] != null && (metadata['full_name'] as String).isNotEmpty) {
          _officerName = metadata['full_name'] as String;
        }
      }
      ...
      if (profile != null) {
        if (profile['full_name'] != null && (profile['full_name'] as String).isNotEmpty) {
          _officerName = profile['full_name'] as String;
        }
        if (profile['email'] != null && (profile['email'] as String).isNotEmpty) {
          _officerEmail = profile['email'] as String;
        }
      ...
      final roleData = await AuthService.fetchRoleData(UserRole.regulator);
      if (roleData != null) {
        if (roleData['department_name'] != null && (roleData['department_name'] as String).isNotEmpty) {
          _departmentName = roleData['department_name'] as String;
        }
        if (roleData['designation'] != null && (roleData['designation'] as String).isNotEmpty) {
          _designation = roleData['designation'] as String;
        }
        if (roleData['official_id'] != null && (roleData['official_id'] as String).isNotEmpty) {
          _officialId = roleData['official_id'] as String;
        }
  ...
  Future<void> _handleSignOut() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to log out of your regulator session?',
      ...
      child: Column(
        children: [
          ...
          _buildInfoRow(
            ...
            value: 'Tier 1 - Audit & Violation Enforcement',
          ),
          _buildDivider(),
          _buildInfoRow(
